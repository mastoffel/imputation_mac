...
read_org <- function(chr) {
    geno_imp <- fread(paste0("data/genos_unimputed_2018/Genotypes_chr_", chr, ".txt"))
    if ((chr != chrs[1])) geno_imp <- geno_imp[, -1]
    setDF(geno_imp)
}
...
chr <- unlist(map(1:length(chrs), function(x) rep(chr_lengths[x], chr_lengths[x])))
df_trans$chr <- chr
